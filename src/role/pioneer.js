...
//Travels to a remote room, then acts similarly to a regular worker
class RolePioneer {
    ...
    static run(creep) {
        ...
        try {
            ...
            if (creep.memory.action == null) {
                ...
                var actionsList = [Actions['FillStorage'], Actions['Travel'], Actions['Repair'], Actions['Build'], Actions['FillSpawn'], Actions['FillExtension'], Actions['UpgradeController'], Actions['Loot'], Actions['Withdraw'], Actions['HarvestEnergy'], Actions['ReturnHome']];
    ...
    //Spawn a new pioneer from spawn using up to maxEnergy energy
    static spawnCreep(spawn, maxEnergy, destRoom) {
        ...
        var creepName = 'Pioneer' + Game.time;
